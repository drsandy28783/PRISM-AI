...
print("Claude Key from ENV:", os.getenv("CLAUDE_API_KEY"))
...
def call_claude(prompt):
    ...
    try:
        response = client.messages.create(
            model="claude-3-5-sonnet-20240620",  # ✅ Official model name
            max_tokens=1000,
            temperature=0.4,
            messages=[
...
@app.route('/test-ai')
def test_ai():
    """Test endpoint to verify Claude AI connection"""
    try:
        test_response = call_claude("Say 'Claude AI is working!' if you can read this.")
        return f"✅ AI Test Result: {test_response}"
    except Exception as e:
